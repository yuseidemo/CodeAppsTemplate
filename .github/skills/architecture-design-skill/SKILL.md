...
│   └─ Q: モデル駆動型アプリ内だけで利用し、Dataverse中心か？
│       ├─ YES → Model-Driven Apps + 生成ページ
│       └─ NO  → Code Apps（shadcn/ui のコンポーネントが活きる）
│
└─ 複雑（カンバン + ガントチャート + インライン編集 + カスタムビジュアル）
  └─ Q: 生成ページの対応範囲（Dataverse・最大6テーブル）に収まるか？
    ├─ YES → Model-Driven Apps + 生成ページを検討
    └─ NO  → Code Apps
```

| 判断ポイント                                         | Code Apps              | Canvas Apps          | Model-Driven Apps               |
| ---------------------------------------------------- | ---------------------- | -------------------- | ------------------------------- |
| カスタム UI コンポーネント                           | ✅ 自由（React + OSS） | ❌ 制約あり          | △ 生成ページはReact + Fluent UI |
...
| ドラッグ&ドロップ（カンバン等）                      | ✅ dnd-kit             | ❌ 非対応            | △ 生成ページで要検証            |
| ガントチャート・ツリー構造                           | ✅ 実装可能            | ❌ 困難              | △ 生成ページで要検証            |
...
> 独自UIが必要でも、モデル駆動型アプリ内で完結し、Dataverse中心かつ1ページ最大6テーブルの制約内なら生成ページを選択できる。
> 生成ページはReact + TypeScriptとFluent UIで実装し、`generative-page-skill` とMicrosoft公式 `/genpage` を使用する。
